...
/*   Updated: 2023/04/01 11:19:10 by oaydemir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			t_grid_spacings grid_spacings)
{
	...
	{
		nearest_boundary_distances->x
			= (ray.position.x - floor(ray.position.x)) * grid_spacings.x;
	}
	else
	{
		nearest_boundary_distances->x
			= (ceil(ray.position.x) - ray.position.x) * grid_spacings.x;
	}
	if (ray.direction.y < 0)
	{
		nearest_boundary_distances->y
			= (ray.position.y - floor(ray.position.y)) * grid_spacings.y;
	}
	else
	{
		nearest_boundary_distances->y
			= (ceil(ray.position.y) - ray.position.y) * grid_spacings.y;
	}
}
